...
class UniqueImageData(Data):
    ...
    def __init__(
        ...
        Args:
            ...
            imaged_count: Dictionary mapping target IDs to their repeat imaging counts.
        """
        if imaged is None:
            imaged = []
        ...
        if imaged_count is None:
            imaged_count = {}
        self.imaged_count = imaged_count

    def add_image(self, target):
        """Increment the imaging count for the given target."""
        if target.id in self.imaged_count:
            self.imaged_count[target.id] += 1
        else:
            self.imaged_count[target.id] = 1
...
class UniqueImageReward(GlobalReward):
    ...
    def calculate_reward(
        ...
        unique_targets = set(imaged_targets)
        for target in unique_targets:
            if target not in self.Completeness:
                self.Completeness[target] = 1  # first image, completeness = 1
        return reward
