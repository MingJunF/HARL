...
class GeneralSatelliteTasking(Env, Generic[SatObs, SatAct]):
    ...
    def __init__(
        ...
        self.prev_target_angles_rad = None  
    ...
    def _get_reward(self):
        """Return the scalar reward and the number of non-zero reward components."""
        base_reward = sum(self.reward_dict.values())
        num_nonzero = sum(1 for r in self.reward_dict.values() if r != 0)

        for satellite in self.satellites:
            if not satellite.is_alive(log_failure=True):
                base_reward += self.failure_penalty

        return base_reward, num_nonzero
    ...
    def step(
        ...
        observation = self._get_obs()
        reward,num_image = self._get_reward()
        ...
        if terminated or truncated:
            ...
            info["completion_rate"] = float(self.num_image / self.num_targets)
            info["reward"] = self.episode_base_reward
        else:
...
class ConstellationTasking(
    ...
    def step(
        ...
        completeness = self.rewarder.Completeness
        observation = self._get_obs()
        ...
        logger.debug(f"Step info: {info}")
        logger.debug(f"Step observation: {observation}")
        return observation, reward, terminated, truncated, info,completeness
